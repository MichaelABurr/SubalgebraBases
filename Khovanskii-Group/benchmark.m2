...
-- Questions from the old test file:
-- What is the answer supposed to be??  Which ring to use?
-- test uses:  R = kk[t,x_1..x_4, MonomialOrder=>Lex, Degrees=>{1,4,3,2,1}]
x = symbol x
...
result := subduction(gens subR, a^3*b);
time assert(result == a*b*c^2-a*b*d^2)
